...
#define MON_OUT_ROWS  20    // visible rows
...
static char     mon_out[MON_OUT_HIST][41];
static int      mon_nout        = 0;  // total lines in history
static int      mon_scroll      = 0;  // 0=bottom, N=scroll back N lines
static int      mon_esc         = 0;  // ESC sequence state: 0=none,1=ESC,2=ESC[
...
static void mon_print(const char *s) {
    if (mon_nout < MON_OUT_HIST) {
        strncpy(mon_out[mon_nout++], s, 40);
        mon_out[mon_nout - 1][40] = '\0';
    } else {
        memmove(mon_out[0], mon_out[1], (MON_OUT_HIST - 1) * 41);
        strncpy(mon_out[MON_OUT_HIST - 1], s, 40);
        mon_out[MON_OUT_HIST - 1][40] = '\0';
    }
}
...
static void cmd_asm_assemble(const char *args) {
    ...
    if (*args) {
        origin = parse_hex(args);
        while (*args && ((*args >= '0' && *args <= '9') ||
                         (*args >= 'A' && *args <= 'F') ||
                         (*args >= 'a' && *args <= 'f'))) args++;
    }
    while (*args == ' ') args++;
    bool listing = (*args == 'L' || *args == 'l');

    // Build flat source buffer from numbered lines (allocated from heap, freed below).
    int src_cap = asm_nlines * (ASM_LINE_LEN + 1) + 1;
    char *src_buf = (char *)malloc((size_t)src_cap);
    if (!src_buf) { mon_print("out of memory"); return; }
    int len = 0;
    for (int i = 0; i < asm_nlines; i++) {
        int n = (int)strlen(asm_lines[i].text);
        if (len + n + 1 >= src_cap) break;
    ...
    extern uint8_t m[];
    int rc = z80asm_assemble(src_buf, len, m, origin, asm_print, listing);
    free(src_buf);
    if (rc < 0)
...
// Generic loader: feed a multi-line C string into the ASM line editor.
// Skips blank lines and pure-comment lines.
static void load_asm_src(const char *src, const char *name) {
    asm_nlines = 0;
    ...
    while (*p && linenum <= 9990) {
        int len = 0;
        while (*p && *p != '\n') {
            if (len < (int)(sizeof(linebuf) - 1)) linebuf[len++] = *p;
            p++;
        }
        if (*p == '\n') p++;
        linebuf[len] = '\0';
    ...
    char buf[41];
    snprintf(buf, sizeof(buf), "%s: %d lines (AS 8000 / G 8000)", name, asm_nlines);
    mon_print(buf);
}
...
// FI          SD card info
// FC          remount SD card
// FX          (no-op, kept for compatibility)

static int fmon_count;  // entry count used in dir callback

static void fl_cb(const char *name, bool is_dir, uint32_t size) {
    char line[41];
    if (is_dir) {
        snprintf(line, sizeof(line), "  %-13s <DIR>", name);
    } else if (size < 1024) {
        snprintf(line, sizeof(line), "  %-13s %lu B", name, (unsigned long)size);
    } else if (size < 1048576) {
        snprintf(line, sizeof(line), "  %-13s %lu K", name, (unsigned long)(size/1024));
    } else {
        snprintf(line, sizeof(line), "  %-13s %lu M", name, (unsigned long)(size/1048576));
    }
    mon_print(line);
    fmon_count++;
}
...
static void cmd_file_list(const char *args) {
    ...
    mon_print(hdr);
    fmon_count = 0;
    if (!sd_fat_dir(path, fl_cb))
        mon_print("not found");
    else if (fmon_count == 0)
        mon_print("  (empty)");
}
...
static void cmd_file_delete(const char *args) {
    ...
    if (!sd_fat_mounted()) { mon_print("no SD card"); return; }
    if (sd_fat_delete(args)) mon_print("deleted");
...
static void cmd_file_mkdir(const char *args) {
    ...
    if (!sd_fat_mounted()) { mon_print("no SD card"); return; }
    if (sd_fat_mkdir(args)) mon_print("created");
...
static void cmd_file_move(const char *args) {
    ...
    while (*sp == ' ') sp++;
    if (!*src || !*sp) { mon_print("usage: FM src dst"); return; }

    if (sd_fat_rename(src, sp)) mon_print("renamed");
    else                        mon_print("error");
}
...
static void cmd_file_binary(const char *args) {
    ...
    while (*sp && *sp != ' ') sp++;
    char path[64];
    int plen = (int)(sp - args);
    if (plen >= (int)sizeof(path)) plen = (int)sizeof(path) - 1;
    memcpy(path, args, (size_t)plen); path[plen] = '\0';
    while (*sp == ' ') sp++;

    uint16_t origin = 0x8000;
    if (*sp) origin = parse_hex(sp);

    if (origin + 0x7FFF < origin) { mon_print("addr out of range"); return; }

    extern uint8_t m[];
    int max = 0x10000 - origin;
    int rc = sd_fat_load(path, m + origin, max);
    char info[41];
    if (rc < 0)
        snprintf(info, sizeof(info), "load failed: %s", path);
    else
        snprintf(info, sizeof(info), "ok: %d bytes at %04X  (G %04X)", rc, origin, origin);
    mon_print(info);
}
...
static void cmd_file_info(const char *args) {
    ...
    char line[41];
    if (!sd_fat_mounted()) {
        mon_print("SD: not mounted  (FC to mount)");
        return;
    }
    uint32_t free_kb = sd_fat_free_kb();
    if (free_kb >= 1024)
        snprintf(line, sizeof(line), "SD: free %lu MB", (unsigned long)(free_kb / 1024));
    else
        snprintf(line, sizeof(line), "SD: free %lu KB", (unsigned long)free_kb);
    mon_print(line);
}
...
static void cmd_airfight(const char *args) {
    ...
    extern uint8_t m[];
    extern uint16_t pc;
    memcpy(&m[0x8000], airfight_bin, airfight_bin_len);
    pc = 0x8000;
    char buf[41];
    snprintf(buf, sizeof(buf), "AIRFIGHT: %u bytes at 8000", airfight_bin_len);
    mon_print(buf);
    mon_print("G to run  (P1:A/D/Space  P2:J/L/M)");
}
...
static const mon_cmd_t commands[] = {
    ...
    { 'P', "P         load AIRFIGHT binary -> 8000", cmd_airfight  },
...
static void cmd_help(void) {
    ...
    mon_print("FB path (addr)=load bin  FC=remount");
    mon_print("FL (path)=list  FK=mkdir FD=del FM=mv");
...
static void mon_exec(const char *cmd) {
    ...
    if (ch == 'F') {
        ...
        if      (sub == 'L') { cmd_file_list(cmd);       return; }
        ...
        else if (sub == 'B') { cmd_file_binary(cmd);     return; }
               ...
               mon_print("   B=bin-load I=info FC=connect"); return; }
    }
...
void monitor_enter(void) {
    abc80_keyboard_clear();   // prevent stale key from blocking input on return
    mon_active  = true;
    mon_nout    = 0;
    mon_scroll  = 0;
    mon_esc     = 0;
    ...
    mon_print(info);
    snprintf(info, sizeof(info), "SD: %s",
             sd_fat_mounted() ? "mounted" : "not mounted (FC to mount)");
    mon_print(info);
}
...
void monitor_serial_poll(void) {
    ...
    // ESC sequence for arrow keys (up/down = scroll)
    if (mon_esc == 1) {
        mon_esc = (ch == '[') ? 2 : 0;
        return;
    }
    ...
    if (ch == 0x1B) { mon_esc = 1; return; }

    if (ch == 0x0D || ch == 0x0A) {
        mon_scroll = 0;   // return to bottom on command entry
        char echo[82];
...
void monitor_render(uint16_t *fb) {
    ...
    // SD state indicator in the middle of the title bar
    fb_draw_string(fb, 15 * 8, 0, sd_fat_mounted() ? "SD:ok " : "SD:-- ",
                   COLOR_BLACK, MON_FG);
    const char *hint = (mon_scroll > 0) ? "^ SCROLL ^  Q=X" : "X/Q=EXIT H=HELP";
    fb_draw_string(fb, 24 * 8, 0, hint, COLOR_BLACK, MON_FG);

    int top = mon_nout - MON_OUT_ROWS - mon_scroll;
    if (top < 0) top = 0;
    int bot = top + MON_OUT_ROWS;
    if (bot > mon_nout) bot = mon_nout;
    for (int i = top; i < bot; i++)
        fb_draw_string(fb, 0, (i - top + 2) * 10, mon_out[i], MON_FG, MON_BG);
