...
// Screen RAM layout
//
// ABC80 uses a 24 row x 40 col display.  The rows are stored non-linearly
...
void abc80_strobe(void) {
    // ABC80 real-time clock   24-bit binary counter, little-endian:
    //   m[0xFDF0]  low byte   (decimal 65008)
    //   m[0xFDF1]  mid byte   (decimal 65009)
    //   m[0xFDF2]  high byte  (decimal 65010)
    //
    // Called at 50 Hz (every 20 ms).  The counter decrements by 1 on every
    // call, so TIME in BASIC reads as  counter x 0.02 seconds elapsed.
    uint32_t clk = (uint32_t)m[0xFDF0]
                 | ((uint32_t)m[0xFDF1] << 8)
                 | ((uint32_t)m[0xFDF2] << 16);
    clk = (clk - 1u) & 0x00FFFFFFu;   // 24-bit decrement, wraps freely
    m[0xFDF0] = (uint8_t)( clk        & 0xFF);
    m[0xFDF1] = (uint8_t)((clk >>  8) & 0xFF);
    m[0xFDF2] = (uint8_t)((clk >> 16) & 0xFF);
...
bool abc80_check_break(void) {
    ...
    break_pending = false;
    return pc >= 0x8000;   // true only when running user code (change to general asm?)
}
