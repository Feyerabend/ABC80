...
// ---------------------------------------------------------------------------
// BAS save as ASCII — detokenizer -- this is not in use!
//
...
static bool bas_emit(uint8_t byte) {
    ...
    if (sim_len >= SIM_BUF_SIZE) {
        if (!sd_fat_save_write(sim_buf, (int)sim_len))
            return false;
        sim_len = 0;
    }
    return true;
...
upload_done:
    ...
    sd_fat_save_end();
    printf("SD: BAS->TEXT  done  %d lines\n", line_count);
}
...
static void sd_handle_open(void) {
    ...
    if (!sd_fat_mounted()) {
        printf("SD: OPEN  no SD card\n");
        sd_error(SD_ERR_NOT_READY);
        return;
    }

    int got = sd_fat_load(sd_fname, sim_buf, (int)SIM_BUF_SIZE);
    if (got < 0) {
        printf("SD: OPEN  %s not found\n", sd_fname);
        sd_error(SD_ERR_NOT_FOUND);
        return;
    }
    sim_len = (uint16_t)got;
    printf("SD: loaded %s  %u B\n", sd_fname, (unsigned)sim_len);
...
static void sd_handle_prepare(void) {
    ...
    if (!sd_fat_mounted()) {
        printf("SD: PREP  no SD card\n");
        sd_error(SD_ERR_NOT_READY);
        return;
    }

    // For BAS LIST writes, open the file now so BL_UT chunks can be streamed.
    // For BAC saves the write happens in CLOSE directly from m[BOFA], so we
    // defer file creation.
    if (sim_file_type == FILE_BAS) {
        if (!sd_fat_save_begin(sd_fname)) {
            printf("SD: PREP  cannot open %s for write\n", sd_fname);
            sd_error(SD_ERR_NOT_READY);
            return;
        }
    }
...
static void sd_handle_close(void) {
    ...
    if (sim_is_writing) {
        ...
        if (sim_file_type == FILE_BAS &&
            ...
            if (remaining > 0) {
                printf("SD: CLOSE  BAS/LIST final %u bytes\n", (unsigned)remaining);
                if (!sd_fat_save_write(m + buf_start, (int)remaining))
                    printf("SD: CLOSE  BAS/LIST final write failed!\n");
            }
            sd_fat_save_end();
            printf("SD: CLOSE  BAS/LIST done\n");

        } else {
            // -----------------------------------------------------------
            // BAC write path (SAVE SD:file.BAC) and BAS SAVE (raw tokens).
            // Close any file opened in PREPARE for the BAS case, then write
            // the raw token bytes from m[BOFA..EOFA-1] as a single call.
            sd_fat_save_end();  // no-op if not open (BAC case skipped save_begin)
            ...
            if (!sd_fat_save(sd_fname, m + bofa, (int)prog_len)) {
                printf("SD: CLOSE  SAVE write failed\n");
                sd_error(SD_ERR_NOT_READY);
                sim_is_writing = false;
                return;
            }
            printf("SD: CLOSE  SAVE done %u bytes\n", (unsigned)prog_len);
        }
...
static void sd_handle_blwrite(void) {
    ...
    if (sim_is_writing && sim_file_type == FILE_BAS) {
        ...
        if (from_list) {
            printf("SD: BL_UT  BAS/LIST +%u bytes\n", (unsigned)data_len);
