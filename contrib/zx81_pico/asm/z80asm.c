...
static Operand parse_operand(Ctx *ctx, Lex *lx) {
    ...
    if (t.type == TT_IDENT) {
        const char *before_ident = lx->p;   /* save: allows re-entry via eval_expr */
        lex_next(lx);
        ...
        /* Symbol expression: restore lexer to before the IDENT so that
         * eval_expr can process the full expression including any trailing
         * operators (e.g. "DFILE >> 8", "AI_RATE / 2", "FOO & 0xFF"). */
        lx->p = before_ident;
        bool undef = false;
        int32_t v = eval_expr(ctx, lx, &undef);
        if (undef && ctx->pass == 2)
            error(ctx, "undefined symbol in expression");
        op.type = OP_IMM; op.val = v; op.undef = undef; return op;
    }
