...
// SL: moved some stuff to a header
#include "z80.h"

/* T-states per main opcode (not-taken base for conditional branches).
 * Prefix bytes CB/DD/ED/FD use fixed approximations. */
static const uint8_t ts_main[256] = {
    /* 00 NOP    01 LD BC,nn  02 LD(BC),A  03 INC BC   04 INC B    05 DEC B    06 LD B,n   07 RLCA */
     4, 10,  7,  6,  4,  4,  7,  4,
    /* 08 EX AF  09 ADD HL,BC 0A LD A,(BC) 0B DEC BC   0C INC C    0D DEC C    0E LD C,n   0F RRCA */
     4, 11,  7,  6,  4,  4,  7,  4,
    /* 10 DJNZ=8 11 LD DE,nn  12 LD(DE),A  13 INC DE   14 INC D    15 DEC D    16 LD D,n   17 RLA  */
     8, 10,  7,  6,  4,  4,  7,  4,
    /* 18 JR=12  19 ADD HL,DE 1A LD A,(DE) 1B DEC DE   1C INC E    1D DEC E    1E LD E,n   1F RRA  */
    12, 11,  7,  6,  4,  4,  7,  4,
    /* 20 JRNZ=7 21 LD HL,nn  22 LD(nn),HL 23 INC HL   24 INC H    25 DEC H    26 LD H,n   27 DAA  */
     7, 10, 16,  6,  4,  4,  7,  4,
    /* 28 JRZ=7  29 ADD HL,HL 2A LD HL,(n) 2B DEC HL   2C INC L    2D DEC L    2E LD L,n   2F CPL  */
     7, 11, 16,  6,  4,  4,  7,  4,
    /* 30 JRNC=7 31 LD SP,nn  32 LD(nn),A  33 INC SP   34 INC(HL)  35 DEC(HL)  36 LD(HL)n  37 SCF  */
     7, 10, 13,  6, 11, 11, 10,  4,
    /* 38 JRC=7  39 ADD HL,SP 3A LD A,(nn) 3B DEC SP   3C INC A    3D DEC A    3E LD A,n   3F CCF  */
     7, 11, 13,  6,  4,  4,  7,  4,
    /* 40-47: LD B,r=4  LD B,(HL)=7 */   4,  4,  4,  4,  4,  4,  7,  4,
    /* 48-4F: LD C,r=4  LD C,(HL)=7 */   4,  4,  4,  4,  4,  4,  7,  4,
    /* 50-57: LD D,r=4  LD D,(HL)=7 */   4,  4,  4,  4,  4,  4,  7,  4,
    /* 58-5F: LD E,r=4  LD E,(HL)=7 */   4,  4,  4,  4,  4,  4,  7,  4,
    /* 60-67: LD H,r=4  LD H,(HL)=7 */   4,  4,  4,  4,  4,  4,  7,  4,
    /* 68-6F: LD L,r=4  LD L,(HL)=7 */   4,  4,  4,  4,  4,  4,  7,  4,
    /* 70-77: LD(HL),r=7  HALT(76)=4 */  7,  7,  7,  7,  7,  7,  4,  7,
    /* 78-7F: LD A,r=4  LD A,(HL)=7 */   4,  4,  4,  4,  4,  4,  7,  4,
    /* 80-87: ADD A,r/m */  4,  4,  4,  4,  4,  4,  7,  4,
    /* 88-8F: ADC A,r/m */  4,  4,  4,  4,  4,  4,  7,  4,
    /* 90-97: SUB r/m   */  4,  4,  4,  4,  4,  4,  7,  4,
    /* 98-9F: SBC A,r/m */  4,  4,  4,  4,  4,  4,  7,  4,
    /* A0-A7: AND r/m   */  4,  4,  4,  4,  4,  4,  7,  4,
    /* A8-AF: XOR r/m   */  4,  4,  4,  4,  4,  4,  7,  4,
    /* B0-B7: OR r/m    */  4,  4,  4,  4,  4,  4,  7,  4,
    /* B8-BF: CP r/m    */  4,  4,  4,  4,  4,  4,  7,  4,
    /* C0 RET NZ=5  C1 POP BC=10  C2 JP NZ=10  C3 JP=10  C4 CALL NZ=10  C5 PUSH=11  C6 ADD n=7  C7 RST=11 */
     5, 10, 10, 10, 10, 11,  7, 11,
    /* C8 RET Z=5   C9 RET=10    CA JP Z=10   CB prefix≈8  CC CALL Z=10  CD CALL=17  CE ADC n=7  CF RST=11 */
     5, 10, 10,  8, 10, 17,  7, 11,
    /* D0 RET NC=5  D1 POP DE=10  D2 JP NC=10  D3 OUT=11  D4 CALL NC=10  D5 PUSH=11  D6 SUB n=7  D7 RST=11 */
     5, 10, 10, 11, 10, 11,  7, 11,
    /* D8 RET C=5   D9 EXX=4     DA JP C=10   DB IN A=11  DC CALL C=10   DD IX≈15   DE SBC n=7  DF RST=11 */
     5,  4, 10, 11, 10, 15,  7, 11,
    /* E0 RET PO=5  E1 POP HL=10  E2 JP PO=10  E3 EX(SP)=19 E4 CALL PO=10 E5 PUSH=11 E6 AND n=7 E7 RST=11 */
     5, 10, 10, 19, 10, 11,  7, 11,
    /* E8 RET PE=5  E9 JP(HL)=4   EA JP PE=10  EB EX DE,HL=4 EC CALL PE=10 ED prefix≈16 EE XOR n=7 EF RST=11 */
     5,  4, 10,  4, 10, 16,  7, 11,
    /* F0 RET P=5   F1 POP AF=10  F2 JP P=10   F3 DI=4    F4 CALL P=10   F5 PUSH=11  F6 OR n=7  F7 RST=11 */
     5, 10, 10,  4, 10, 11,  7, 11,
    /* F8 RET M=5   F9 LD SP,HL=6 FA JP M=10   FB EI=4    FC CALL M=10   FD IY≈15   FE CP n=7  FF RST=11 */
     5,  6, 10,  4, 10, 15,  7, 11,
};
...
SI void ccall(bv c) { u16 t = p16(); if (c) { call(t); _cond_taken = 1; } wz = t; }
SI void ret() { jmp(pop()); }
SI void cret(bv c) { if (c) { ret(); _cond_taken = 1; } }

// Relative jumps.
SI void jr(i8 disp) { wz = pc += disp; }
SI void cjr(bv c) { if (c) { jr(p8()); _cond_taken = 1; } else pc++; }
...
SI void ini() { w8(get_hl(), port_in(b, c)); adji(); }
...
/* Jump directly into the ROM's post-load routine at 0x0207, bypassing cold-start.
 * Matches the JS/TS reference: reset registers, then set the four key registers. */
...
void step() {
  _cond_taken = 0;
  u8 opc = halted ? 0x00 : p8();
  exec(opc);

  z80_ts = ts_main[opc];
  if (_cond_taken) {
      if (opc == 0x10 || opc == 0x20 || opc == 0x28 || opc == 0x30 || opc == 0x38)
          z80_ts += 5;   /* DJNZ / JR cc taken: +5 */
      else if ((opc & 0xC7) == 0xC4)
          z80_ts += 7;   /* CALL cc taken: +7 */
      else
          z80_ts += 6;   /* RET cc taken: +6 */
  }
...
SI void exec(u8 opc) {
  ...
  // in/out
  H(0xDB, t3=a;a=port_in(t3,p8());wz=(t3<<8)|(a+1))
